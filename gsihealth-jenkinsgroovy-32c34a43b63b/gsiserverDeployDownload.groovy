            ...
            stage('Deploy') {
                ...
                if (_proceed) {
                    ...
                    for (def i = 0; i < reqDeployApps.size(); i++) {
                        ...
                        PS_deploy += spring ? "Deploy-SpringModulesDownload.ps1 " : nodeApp ? "Deploy-NodeModulesBackup.ps1 " : "Deploy-Modules.ps1 "
                        PS_deploy += "-env '${environment}' -server '${server}' -appList @('${appName}') -sourceDir ${workspace} -release '${_release}' -buildName '${buildName}' -serverName '${serverName}'"
